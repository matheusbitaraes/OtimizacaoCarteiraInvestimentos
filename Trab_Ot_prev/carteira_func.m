function [EstMdl,EstParamCov,logL,info] = carteira_func(serie_historica)
    ...
    dY = diff(serie_historica,2); % Diferenciando remove a tendência linear dos dados, o que indica processo não-estacionário
    PC = parcorr(dY,25); % 25 valores de atraso para a função de autocorrelação parcial
    
    AC = autocorr(dY,25); %25 valores de atraso para a função de autocorrelação
